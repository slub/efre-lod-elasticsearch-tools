...
if __name__ == "__main__":
    ...
    ef_instances = ["http://"+args.host+":"+args.port +
    ...
    if args.stdin:
        for line in sys.stdin:
            ...
            record = None
            if rec.get("sameAs"):
                if isinstance(rec.get("sameAs"), list) and any("https://d-nb.info"in x for x in rec.get("sameAs")):
        ...
        for rec in esgenerator(host=args.host, port=args.port, index=args.index, type=args.type, headless=True, body={"query": {"prefix": {"sameAs.keyword": "https://d-nb.info"}}}, verbose=True):
            gnd = None
            if rec.get("sameAs"):
                if isinstance(rec.get("sameAs"), list) and any("https://d-nb.info" in x for x in rec.get("sameAs")):
                    for item in rec.get("sameAs"):
