...
class LODFINCTITDownloadRawData(LODFINCTITTask):
    def run(self):
        ...
        solrdump_cmd+="  -q 'institution:DE-15 last_indexed:[{last} TO {now}]' | ~/git/efre-lod-elasticsearch-tools/processing/finc2rdf.py | {bzip} > {date}-finc.ldj.bz2".format(last=lu,now=self.now,host=self.config.get("url"),bzip=get_bzipper(),date=self.date)
        print(solrdump_cmd)
