...
def compact_object(jsonobject):
    ...
    if isinstance(jsonobject, dict):
        if (record_field and record_field in jsonobject) or (record_field is None):
            compacted = jsonld.compact(jsonobject, context,  {
                                       'skipExpansion': True})
            if context_url:
            ...
            for date in ["definition"]:
                ...
                if isinstance(compacted.get("gndIdentifier"), list):
                    compacted["gndIdentifier"] = compacted.pop("gndIdentifier")[
                        0]
            ...
            if (node and compacted.get("@id") and compacted.get("@id").startswith("_:")) or (node and compacted.get("id") and compacted.get("id").startswith("_:")):
                ...
                with gzip.open(pathprefix+str(current_process().name)+"-bnodes.ldj.gz", "at") as fileout:
...
def process(inputs, record_field, context_url, pathprefix, bnode, worker):
    ...
    if r.ok:
        ...
        sys.stderr.write(
            "unable to get context from {}. aborting\n".format(context_url))
        exit(-1)

    pool = Pool(worker, initializer=init_mp, initargs=(
        jsonldcontext, record_field, context_url, pathprefix, bnode,))
...
class GNDcompactedJSONdata(GNDTask):
    ...
    def run(self):
        ...
        process(self.config.get("urls"), None,
                self.config.get("context"), "chunks/", True, 28)
